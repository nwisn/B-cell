...
g <- ggplot(this_df) + 
        ...
        scale_color_jco(name = "Cluster") + scale_fill_jco(name = "Cluster") + 
        theme_classic() 
print(g)
...
annotation.colors$bicluster <- bicluster$annotation.colors
...
shortlist$cluster <- cluster[match(shortlist$biomarker, names(cluster))]
...
this_table <- table(category = shortlist$parameter, cluster = shortlist$cluster)
kable(this_table) %>% 
    kable_styling(bootstrap_options = c("striped", 
                                        "hover", 
                                        "condensed",
