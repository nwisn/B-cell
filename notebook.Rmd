---
title: "Analysis of immune biomarkers in MCS device implantation"
subtitle: "B-cells and cytokines"
...
We double-standardized ( [Efron 2009](https://projecteuclid.org/euclid.aoas/1254773272)) the data and biclustered it using the Euclidean distance. 
...
After double-standardizing the data, we clustered the biomarkers according to their temporal patterns. They form 4 main clusters.

```{r temporal_clusters, warning=F, message=F, fig.height=9}
df.ds <- df.HMII
...
annotation.row <- data.frame("Molecule" = factor(c(rep("B-cell",29), rep("cytokine",38))))
...
annotation.row$Cluster <- cluster
annotation.colors$Cluster <- colorfun(nlevels(factor(annotation.row$Cluster)))
names(annotation.colors$Cluster) <- factor(unique(annotation.row$Cluster))
...
We depicted each cluster's temporal pattern as a smooth function of time, using a LOESS model. They reveal clear temporal dynamics occurring at multiple timescales.
...
ggplot(df.clustermeans) + 
    ...
    stat_smooth(method = "loess", aes(group = cluster), span = .8) +
